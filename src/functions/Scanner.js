...
export class Scanner extends Component {
    ...
    handleManualScan() {
        if (this.state.manualResult) {
            this.props.change(this.state.manualResult);
        }
    }
    ...
    getOut = () => {
        if (this.props.history) {
            ...
            console.log("varken history eller close verkar finnas, kan ej backa.");
        }
    }
            ...
            <div>
                <Grid container direction="row" justify="space-between">
                    ...
                    <Button color="primary" fontSize="large" onClick={val => { this.setState({ isQr: !this.state.isQr }) }}>
                        {this.state.isQr ? "QR-kod" : "Streckkod"}
                    </Button>
                ...
                <Grid container direction="row" justify="space-between">
                    <Form.Group style={this.state.manualResult === '' ? { width: '100%' } : { width: '75%' }}>
                        <Form.Control type="text" name="name" placeholder="Här kan du skriva in koden manuellt"
                            onChange={val => { this.setState({ manualResult: val.target.value }); }} />
                    </Form.Group>
                    {this.state.manualResult !== '' ?
                        <Button color="primary" variant="outlined" style={{ height: "40px", width: "50px" }} onClick={val => {this.handleManualScan() }}>
                            OK
                        </Button>
                    : ""}
                </Grid>
